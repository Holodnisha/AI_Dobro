...
class MyDataset(Dataset):
    def __init__(self, csv_file):
        ...
        self.vectorizer = CountVectorizer(max_features=32768)
        if LearnTask and vectorize_num:
            self.vectorizer.fit(self.data['Description'].apply(self.preprocess_text))
            joblib.dump(self.vectorizer, "vectorizer_0915_6.pkl")
            print("save vectorizer")
        else:
            print("load vectorizer")
            self.vectorizer = joblib.load("vectorizer_0915_6.pkl")
...
def validate(model, data_loader, criterion, epoch):
    ...
    if best_loss is None:
        best_loss = avg_loss
    elif avg_loss < best_loss - 0.0002:
        best_loss = avg_loss
        count = 0
        save_model(model, 'model_0915_6.pth', total_loss, total_loss / len(data_loader), best_loss, train_acc, accuracy, epoch)
...
if __name__ == "__main__":
    if LearnTask:
        ...
        model = MyModel(
            input_size=32768,
            hidden_size=1024,  # Нормальный размер
            output_size=9,
            num_layers=16  # Уменьшенное количество слоев
        ).to(device)
        ...
        loaded_model = load_model('model_0915_6.pth', 'data_true_PRED.xlsx', 'data_true_PRED.csv')
