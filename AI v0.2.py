...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        global LearnTask, unique_words, counter
        ...
        if self.counter >= 41050 and epoch == 1:
...
if __name__ == "__main__":
    if LearnTask:
        filen = '____.txt'
        xlsx_file_train = 'Data_New.xlsx'
        csv_file_train = 'Data_New.csv'
     ...
     #   test_loss, test_results = test(model, test_loader, criterion)
        ...
        filen = '____.txt'
        load_set_from_file(filen)
        print(f'Уникальные слова загружены из файла: {loaded_set}')
        loaded_model = load_model('model.pth','Data_New.xlsx','Data_New.csv')
        prediction = predict(loaded_model, 'Data_New.xlsx', 'Data_New.csv')
