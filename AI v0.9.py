...
class MyDataset(Dataset):
    def __init__(self, csv_file):
        ...
        if LearnTask == 1:
            ...
            self.vectorizer = joblib.load("vectorizerTEST.pkl")
...
def validate(model, data_loader, criterion, epoch):
    ...
    if best_loss == None:
        ...
        best_loss = total_loss / len(data_loader)
        count = 0
        save_model(model, 'modelTEST.pth', loss_train, total_loss / len(data_loader), best_loss, train_acc, accuracy, epoch)
...
if __name__ == "__main__":
    if LearnTask:
        ...
        model = MyModel(16384, 9).to(device)
        ...
        loaded_model = load_model('modelTEST.pth','data_work.xlsx','data_work.csv')
