...
class MyModel(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.act4 = nn.ReLU()
        self.fc5 = nn.Linear(8192, 16384)
        self.act5 = nn.ReLU()
        self.fc6 = nn.Linear(16384, 8192)
        self.act6 = nn.ReLU()
...
def train(model, data_loader, criterion, optimizer):
    global loss_train, train_acc, epochs, tech_epoch, tech_state
    model.train()
...
def validate(model, data_loader, criterion, epoch):
    model.eval()
    global br_p, best_loss, count, tech_state
    total_loss = 0
    total_correct = 0
    total_samples = 0
    tech_state = False
...
if __name__ == "__main__":
    if LearnTask:
        ...
        lr = 0.001
        epochs = 3000
        train_dataset = MyDataset(csv_file_train)
