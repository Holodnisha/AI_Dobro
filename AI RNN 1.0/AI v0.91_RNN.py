...
def validate(model, data_loader, criterion, epoch):
    ...
    if best_loss is None:
        ...
        best_loss = avg_loss
        count = 0
        save_model(model, 'model_0915_1.pth', total_loss, total_loss / len(data_loader), best_loss, train_acc, accuracy,
                   epoch)
...
if __name__ == "__main__":
    if LearnTask:
        ...
        model = MyModel(
            input_size=16384,
            hidden_size=512,  # Уменьшенный размер
            output_size=9,
            num_layers=2  # Уменьшенное количество слоев
        ).to(device)
        ...
        loaded_model = load_model('model_0915_1.pth', 'data_work.xlsx', 'data_work.csv')
